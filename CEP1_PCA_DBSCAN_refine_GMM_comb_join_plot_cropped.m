...
join_cl_res_file = 'E:\Septin_structure_analysis\updated_pictures\data7\CEP1_structures\PCA_DBSCAN_join.mat';
...
w = 4;
loc = [bLoc{uu} ulab{w}];
mkdir([loc '_PCA_DBSCAN_refine_GMM_comb_plot_join_crop']);
fr_loc = [1680 570];
load([loc '.mat']);
...
for cl_id = 1:cl_num
    ...
    drawnow;
    saveas(fig, [loc '_PCA_DBSCAN_refine_GMM_comb_plot_join_crop', '\cluster=' num2str(cl_id) '.png']);
...
fig.InvertHardcopy = 'off';
drawnow;
saveas(fig, [loc '_PCA_DBSCAN_refine_GMM_comb_plot_join_crop', '\all selected clusters.png']);
